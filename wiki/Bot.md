...
- 输入关键词搜索影巢资源。
- 可以直接点击结果按钮，或输入结果编号后，Bot 会继续查询该条目的天翼资源。
- 如果资源已解锁或免费，可以直接点击资源按钮，或输入资源编号进入创建任务流程。
- 如果资源需要积分，Bot 会先显示确认按钮，确认后再调用影巢解锁。
- `/cancel` 退出搜索模式。
- 3 分钟无操作自动退出。

如果当前正处于影巢搜索模式，直接发送 `cloud.189.cn` 分享链接时，Bot 会自动退出影巢模式并改为处理该分享链接。
